...
按单词读取，当单次开头为 $ 时，判断单词后续内容是否为数字，如果是则计算减免之后的价格。
    ...
    public String discountPrices(String sentence, int discount) {
        ...
        for (int i = 0; i < n; i++) {
            if (sentence.charAt(i) == '$') {
                ...
                if (isNumber && word.length() > 0) {
                    String price = String.format("%.2f", Long.parseLong(word.toString()) * (100 - discount) / 100.0);
                    ans.append(price);
                } else {
                    ans.append(word);
                }
            ...
            if (i < n) ans.append(sentence.charAt(i));
        }
        return ans.toString();
    }
}
