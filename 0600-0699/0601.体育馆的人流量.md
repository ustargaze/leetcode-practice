...
参考 [图解——连续日期及难点分析](https://leetcode.cn/problems/human-traffic-of-stadium/solutions/701681/tu-jie-lian-xu-ri-qi-ji-nan-dian-fen-xi-xnj58)

**代码**

```mysql
with t as (select *, id - row_number() over(order by id) as rk from stadium where people >= 100)

select id, visit_date, people
from t
where rk in (select rk from t group by rk having count(rk) >= 3)
```
