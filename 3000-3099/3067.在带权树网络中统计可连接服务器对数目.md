...
假设节点 root 有 4 棵子树并且其中到根节点的距离可以被 signalSpeed 整除的节点数量分别为 a、b、c、d，则联通对为 a * b + a * c + a * d + b * c + b * d + c * d = 0 * a + a * b + (a + b) * c + (a + b + c) * d，每次计算出一棵子树的满足条件的节点数量时与之前的节点数量总和即为新增的结果。

枚举所有的节点作为根节点，然后通过 DFS 遍历每个子树中到根节点的距离可以被 signalSpeed 整除的节点数量计算结果。
...
class Solution {
    public int[] countPairsOfConnectableServers(int[][] edges, int signalSpeed) {
        ...
        for (int i = 0; i < n; i++) {
            int cnt = 0;
            for (int[] e : g[i]) {
                int r = dfs(e[0], i, e[1], g, signalSpeed);
                ans[i] += cnt * r;
                cnt += r;
            }
        }
    ...
    private int dfs(int cur, int p, int dis, List<int[]>[] g, int signalSpeed) {
        int res = 0;
        ...
        for (int[] e: g[cur]) {
            if (e[0] != p) {
                res += dfs(e[0], cur, dis + e[1], g, signalSpeed);
            }
        }
        return res;
    }
